...
def get_coords(address):
    try:
        geolocator = Nominatim(user_agent="giro_visite_agente_v26")
            ...
            with c1:
                for t in tappe:
                    with st.container(border=True):
                        ...
                        if cs.button("🚫", key=f"sk_{t['nome cliente']}"):
                        ...
                        with st.expander("📝 Report Visita"):
                            nota = st.text_area("Note", value=t.get('note', ''), key=f"nt_{t['nome cliente']}")
                            if st.button("💾 Salva", key=f"sv_{t['nome cliente']}"):
                                idx_m = st.session_state.df_master[st.session_state.df_master['nome cliente'] == t['nome cliente']].index[0]
                                st.session_state.df_master.at[idx_m, 'note'] = nota
...
# --- TAB: MAPPA ---
    ...
    if not df_m.empty:
        ...
        for _, row in df_m.iterrows():
            ...
            folium.Marker([row['latitude'], row['longitude']], popup=row['nome cliente'], tooltip=row['nome cliente'], icon=folium.Icon(color=c, icon="user", prefix="fa")).add_to(m)
        ...
        if cl:
            ...
            else: st.session_state.last_map_click = cl; st.toast(f"Tocca di nuovo: {cl}")
...
# --- TAB: ANAGRAFICA ---
    ...
    if scelto != "":
        ...
        with st.form("edit_an"):
            ...
            sv = c1.selectbox("Includere?", ["SI", "NO"], index=0 if d['visitare'] == "SI" else 1)
            ut = c2.text_input("Telefono", d.get('telefono', ''))
            uc = c2.text_input("Cellulare", d.get('cellulare', ''))
            um = c2.text_input("Email", d.get('mail', ''))
            st.divider()
            ad = c1.date_input("Data Appuntamento", value=d['appuntamento'].date() if pd.notnull(d['appuntamento']) else None)
            at = c1.time_input("Ora Appuntamento", value=d['appuntamento'].time() if pd.notnull(d['appuntamento']) else time(10, 0))
            rm = c2.checkbox("Rimuovi appuntamento")
            no = st.text_area("Note", d.get('note', ''))
        ...
        if st.button("❌ ELIMINA CLIENTE"):
            ...
            st.session_state.cliente_selezionato = None; save_to_gsheets(st.session_state.df_master); st.rerun()
...
# --- TAB: NUOVO CLIENTE ---
    ...
    if st.button("🎯 USA GPS ATTUALE"):
        pos = streamlit_js_eval(js_expressions='navigator.geolocation.getCurrentPosition((pos) => { return pos.coords; })', target_id='gps_n')
        if pos: st.session_state.new_coords = (pos['latitude'], pos['longitude'])
    with st.form("new_cl"):
        c1, c2 = st.columns(2)
        nn, nf = c1.text_input("Ragione Sociale *"), c1.number_input("Frequenza (gg)", value=30)
        ut, uc, um = c2.text_input("Telefono"), c2.text_input("Cellulare"), c2.text_input("Email")
        st.divider(); vi = st.text_input("Via e Civico")
        cr = st.columns([1, 2, 1]); cap, cit, pro = cr[0].text_input("CAP"), cr[1].text_input("Città *"), cr[2].text_input("Prov.")
        mlat, mlon = st.text_input("Lat manuale"), st.text_input("Lon manuale")
        if st.form_submit_button("✅ SALVA"):
            if nn and cit:
                ind = f"{vi}, {cap} {cit} {pro}".strip(", ")
                lat, lon = (float(mlat), float(mlon)) if mlat and mlon else (st.session_state.new_coords if 'new_coords' in st.session_state else (None, None))
                if not lat:
                    with st.spinner("Cerco..."):
                        lat, lon = get_coords(ind) or get_coords(f"{vi}, {cit}") or get_coords(cit) or (None, None)
                if lat:
                    nuovo = {'nome cliente': nn, 'indirizzo': ind, 'referente': '', 'telefono': ut, 'cellulare': uc, 'mail': um, 'note': '', 'visitare': 'SI', 'frequenza (giorni)': nf, 'latitude': lat, 'longitude': lon, 'ultima visita': pd.Timestamp('2000-01-01'), 'appuntamento': pd.NaT}
...
# --- TAB: PARAMETRI ---
    ...
    st.subheader("📍 Partenza")
    if st.button("🎯 USA GPS"):
        gps = streamlit_js_eval(js_expressions='navigator.geolocation.getCurrentPosition((pos) => { return pos.coords; })', target_id='gps_p')
        if gps: save_config_cloud("GPS", gps['latitude'], gps['longitude']); st.rerun()
    nc = st.text_input("Città:", st.session_state.start_city)
    if nc != st.session_state.start_city:
        co = get_coords(nc); (save_config_cloud(nc, co[0], co[1]) and st.rerun()) if co else None
    st.divider()
    c1, c2 = st.columns(2); st.session_state.h_inizio = c1.time_input("Inizio", st.session_state.h_inizio)
    st.session_state.h_fine = c2.time_input("Fine", st.session_state.h_fine); st.session_state.durata_v = st.slider("Minuti visita", 15, 120, st.session_state.durata_v)
    f_in = st.date_input("Ferie", value=st.session_state.ferie)
    st.session_state.ferie = f_in if isinstance(f_in, tuple) and len(f_in) == 2 else st.session_state.ferie
...
# --- TAB: AGENDA ---
    ...
    if col_p.button("⬅️", disabled=(st.session_state.current_week_index == 0)): st.session_state.current_week_index -= 1; st.rerun()
    ...
    if col_n.button("➡️", disabled=(st.session_state.current_week_index == 10)): st.session_state.current_week_index += 1; st.rerun()
    st.divider(); cols = st.columns(5); g_n = ["Lun", "Mar", "Mer", "Gio", "Ven"]
