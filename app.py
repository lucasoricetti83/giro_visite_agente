            ...
            with c1:
                for t in tappe:
                    with st.container(border=True):
                        ...
                        cols = st.columns(4)
                        cols[0].link_button("🚗 Naviga", f"https://www.google.com/maps/dir/?api=1&destination={t['latitude']},{t['longitude']}")
                        if t.get('cellulare'): cols[1].link_button("📞 Cell", f"tel:{t['cellulare']}")
                        if t.get('mail'): cols[2].link_button("📧 Mail", f"mailto:{t['mail']}")
                        if cols[3].button("👤 Info", key=f"go_{t['nome cliente']}"):
...
# --- TAB: MAPPA CLIENTI ---
    ...
    st.header("🗺️ Analisi Geografica Clienti")
    with st.container(border=True):
        c1, c2 = st.columns([1, 1])
        filtro_tipo = c1.radio("Filtro visualizzazione:", ["Tutti i Clienti", "Visitati", "Mai Visitati", "Singolo Cliente"], horizontal=True)
        df_mappa = st.session_state.df_master.copy()
        if filtro_tipo == "Visitati":
            df_mappa = df_mappa[df_mappa['ultima visita'] > pd.Timestamp('2000-01-01')]
        elif filtro_tipo == "Mai Visitati":
            df_mappa = df_mappa[(df_mappa['ultima visita'].isna()) | (df_mappa['ultima visita'] <= pd.Timestamp('2000-01-01'))]
        elif filtro_tipo == "Singolo Cliente":
            cliente_filtro = c2.selectbox("Scegli il cliente:", sorted(df_mappa['nome cliente'].unique()))
            df_mappa = df_mappa[df_mappa['nome cliente'] == cliente_filtro]

    if not df_mappa.empty:
        df_mappa['color'] = df_mappa['visitare'].apply(lambda x: "#28a745" if x == "SI" else "#dc3545")
        st.map(df_mappa, latitude='latitude', longitude='longitude', color='color', size=25)
        st.caption("🟢 Verde: Inclusi | 🔴 Rosso: Esclusi")
...
# --- TAB: PARAMETRI (RIPRISTINATA COMPLETAMENTE) ---
    ...
    with c_city:
        ...
        if nc != st.session_state.start_city and nc != "Posizione GPS":
            ...
            if coords:
                if save_config_cloud(nc, coords[0], coords[1]):
    ...
    # 2. ORARI DI LAVORO
    st.subheader("⏰ Orari Giornalieri")
    col_ora1, col_ora2 = st.columns(2)
    st.session_state.h_inizio = col_ora1.time_input("Inizio Lavoro", st.session_state.h_inizio)
    st.session_state.h_fine = col_ora2.time_input("Fine Lavoro", st.session_state.h_fine)

    # 3. PAUSA PRANZO
    st.subheader("🍽️ Pausa Pranzo")
    col_p1, col_p2 = st.columns(2)
    st.session_state.pausa_inizio = col_p1.time_input("Inizio Pausa", st.session_state.pausa_inizio)
    st.session_state.pausa_fine = col_p2.time_input("Fine Pausa", st.session_state.pausa_fine)
    
    st.session_state.durata_v = st.slider("Minuti medi per visita", 15, 120, st.session_state.durata_v)

    st.divider()

    # 4. FERIE (RANGE DI DATE)
    st.subheader("🏖️ Gestione Ferie / Chiusura")
    ferie_input = st.date_input("Seleziona periodo di ferie (Inizio e Fine)", value=st.session_state.ferie)
    if isinstance(ferie_input, tuple) and len(ferie_input) == 2:
        st.session_state.ferie = ferie_input

    st.divider()

    # 5. EXPORT E RICARICAMENTO
    ...
    c_ex1.download_button("📥 Scarica Excel", data=to_excel(st.session_state.df_master), file_name="crm_giro.xlsx")
    if c_ex2.button("🔄 Forza Ricaricamento Cloud"):
        st.cache_data.clear()
        st.rerun()
...
# --- ALTRE TAB ---
    ...
    with st.form("new"):
        ...
        ni = st.text_input("Indirizzo Completo *")
        if st.form_submit_button("✅ Crea"):
            if nn and ni:
                coords = get_coords(ni)
                if coords:
                    nuovo = {'nome cliente': nn, 'indirizzo': ni, 'visitare': 'SI', 'frequenza (giorni)': 30, 'latitude': coords[0], 'longitude': coords[1], 'ultima visita': pd.Timestamp('2000-01-01')}
