...
# Inizializzazione ferie vuota per non bloccare il giorno corrente
...
def calcola_piano():
    ...
    for s in range(11):
        for g in range(5):
            ...
            # Controllo ferie (solo se il range è impostato correttamente)
...
for i, m in enumerate(menu):
    if nav[i].button(m, key=f"btn_{m}", use_container_width=True, type="primary" if st.session_state.active_tab == m else "secondary"):
...
# --- TAB: MAPPA ---
    ...
    filtro_tipo = st.radio("Filtro clienti:", ["Tutti i Clienti", "Visitati", "Mai Visitati"], horizontal=True, key="filtro_mappa")
...
# --- TAB: ANAGRAFICA ---
    ...
    scelto = st.selectbox("Cerca cliente:", opzioni_ricerca, index=idx_default, key="search_anag")
    ...
    if scelto != "":
        ...
        # FUNZIONE: APPENA VISITATO (DATA VARIABILE + STORICO)
        with st.container(border=True):
            ...
            if st.session_state.get('show_quick_report', False):
                ...
                report_t = st.text_area("Cosa è emerso?", placeholder="Scrivi il report...", key="quick_rep_area")
        ...
        with st.form("edit_anag"):
            st.subheader("Modifica Anagrafica")
            c1, c2 = st.columns(2)
    ...
    with st.form("new_cl_form"):
        ...
        if st.form_submit_button("✅ CREA E SALVA"):
            if nn and cit:
                ...
                if lat_lon:
                    nuovo = {'nome cliente': nn, 'indirizzo': indirizzo, 'contatto': nco, 'visitare': 'SI', 'frequenza (giorni)': nf, 'latitude': lat_lon[0], 'longitude': lat_lon[1], 'ultima visita': pd.Timestamp('2000-01-01'), 'telefono': ut, 'cellulare': uc, 'mail': um}
...
# --- TAB: PARAMETRI ---
    ...
    # Il campo può essere lasciato vuoto ora
    ferie_in = st.date_input("Seleziona periodo (lascia vuoto se non ci sono ferie):", value=st.session_state.ferie)
    ...
    def to_excel(df):
        ...
        return out.getvalue()
    st.download_button(label="📥 Scarica Database Excel", data=to_excel(st.session_state.df_master), file_name="crm_giro.xlsx")
