...
# Streamlit Cloud usa UTC. Aggiungiamo 1 ora per allinearci all'Italia (CET).
ora_italiana = datetime.now() + timedelta(hours=1)

# --- FUNZIONI DI SALVATAGGIO ---
def save_to_gsheets(df):
    try:
        # Pulizia colonne temporanee prima del salvataggio
...
def fetch_data():
    try:
        ...
        # Assicuriamo che tutte le colonne necessarie siano presenti
...
# MODIFICA RICHIESTA: Inizializzazione ferie vuota per non bloccare il giorno corrente
...
for i, m in enumerate(menu):
    if nav[i].button(m, key=f"nav_btn_{m}", use_container_width=True, type="primary" if st.session_state.active_tab == m else "secondary"):
...
# --- TAB: MAPPA ---
    ...
    filtro_tipo = st.radio("Filtro clienti:", ["Tutti i Clienti", "Visitati", "Mai Visitati"], horizontal=True, key="filtro_map_pro")
...
# --- TAB: ANAGRAFICA ---
    ...
    scelto = st.selectbox("Cerca cliente:", opzioni_ricerca, index=idx_default, key="search_anag_pro")
    ...
    if scelto and scelto != "":
        ...
        # FUNZIONE: APPENA VISITATO (DATA VARIABILE + STORICO REPORT)
        with st.container(border=True):
            st.subheader("🏁 Azione Rapida Fine Visita")
            if st.button("✅ APPENA VISITATO", type="primary", use_container_width=True, key="btn_visited_pro"):
                st.session_state.show_quick_report = True
            
            if st.session_state.get('show_quick_report', False):
                cr1, cr2 = st.columns([1, 2])
                data_v = cr1.date_input("Data della visita:", value=ora_italiana.date(), key="date_visit_pro")
                report_t = st.text_area("Cosa è emerso dalla visita?", placeholder="Scrivi qui il report...", key="text_report_pro")
                
                b_col1, b_col2 = st.columns(2)
                if b_col1.button("💾 SALVA REPORT IN STORICO", use_container_width=True, key="save_report_pro"):
                    ...
                    if save_to_gsheets(st.session_state.df_master):
                        st.success("Visita registrata!"); st.session_state.show_quick_report = False; st.rerun()
                if b_col2.button("Annulla", use_container_width=True, key="cancel_report_pro"):
                    st.session_state.show_quick_report = False; st.rerun()

        st.divider()

        with st.form("edit_anag_pro"):
            c1, c2 = st.columns(2)
            ...
            scelta_v = c1.selectbox("Includere?", ["SI", "NO"], index=0 if d['visitare'] == "SI" else 1)
            ...
            st.subheader("📝 Note (Annotazioni fisse)")
            uno = st.text_area("Annotazioni fisse", d.get('note', ''), height=100)
        ...
        with st.expander("🗑️ ELIMINA CLIENTE"):
            if st.checkbox("Confermo eliminazione") and st.button("❌ ELIMINA ORA", key="del_btn_pro"):
    ...
    with st.form("new_cl_full_pro"):
        ...
        if st.form_submit_button("✅ CREA E SALVA"):
            if nn and cit:
                ...
                if lat_lon:
                    nuovo = {'nome cliente': nn, 'indirizzo': ind_comp, 'contatto': nco, 'visitare': 'SI', 'frequenza (giorni)': nf, 'latitude': lat_lon[0], 'longitude': lat_lon[1], 'ultima visita': pd.Timestamp('2000-01-01'), 'appuntamento': pd.NaT}
...
# --- TAB: PARAMETRI ---
    ...
    st.subheader("🏖️ Periodo Ferie / Chiusura")
    # MODIFICA: Il campo ferie ora può essere lasciato vuoto
    ferie_in = st.date_input("Seleziona periodo (lascia vuoto se non ci sono ferie):", value=st.session_state.ferie, key="ferie_input_pro")
    ...
    def to_excel(df):
        ...
        return out.getvalue()
    st.download_button(label="📥 Scarica Database Excel", data=to_excel(st.session_state.df_master), file_name="crm_giro.xlsx", key="btn_download_pro")
    if st.button("🔄 Forza Ricarica Cloud", key="reload_cloud_pro"): st.cache_data.clear(); st.rerun()
...
# --- TAB: AGENDA ---
    ...
    if col_prev.button("⬅️ Precedente", disabled=(st.session_state.current_week_index == 0), key="agg_prev"):
    ...
    if col_next.button("Successiva ➡️", disabled=(st.session_state.current_week_index == 10), key="agg_next"):
        st.session_state.current_week_index += 1; st.rerun()
    st.divider()
    cols = st.columns(5)
        ...
        with cols[i]:
            ...
            for t in agenda[st.session_state.current_week_index][i]:
                with st.container(border=True):
                    ...
                    if st.button(t['nome cliente'], key=f"btn_agg_pro_{st.session_state.current_week_index}_{i}_{t['nome cliente']}", use_container_width=True):
