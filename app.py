...
def get_coords(address):
    try:
        geolocator = Nominatim(user_agent="giro_visite_v13_pro")
...
# --- 3. STATO DELL'APP (PARAMETRI PERSISTENTI) ---
...
# Inizializzazione orari e parametri se non presenti
for key, val in {
    ...
    for s in range(1, 9):
        for g in range(5):
            ...
            while True:
                ...
                p_in = datetime.combine(dt_c, st.session_state.pausa_inizio)
                p_fi = datetime.combine(dt_c, st.session_state.pausa_fine)
                if arr >= p_in and arr < p_fi:
                    o_s = p_fi; continue
...
# --- TAB: ANAGRAFICA ---
    ...
    if scelto:
        ...
        with st.form("edit_anag"):
            ...
            app_d = c1.date_input("Data", value=d['appuntamento'].date() if pd.notnull(d['appuntamento']) else None)
        ...
        with st.expander("🗑️ ELIMINA CLIENTE"):
            ...
            conf = st.checkbox("Confermo cancellazione")
            if st.button("❌ ELIMINA ORA") and conf:
                st.session_state.df_master = st.session_state.df_master.drop(idx)
                if save_to_gsheets(st.session_state.df_master): st.rerun()

# --- TAB: NUOVO CLIENTE (INDIRIZZO DETTAGLIATO + GPS) ---
elif st.session_state.active_tab == "➕ Nuovo Cliente":
    st.header("➕ Registrazione Nuovo Cliente")
    st.markdown("### 📍 Rileva posizione GPS attuale")
    pos_new = streamlit_js_eval(js_expressions='navigator.geolocation.getCurrentPosition((pos) => { return pos.coords; })', target_id='gps_new_v13')
    ...
    with st.form("new_full"):
        ...
        via_n = st.text_input("Via e Civico")
        ca_row = st.columns([1, 2, 1])
        cap_n = ca_row[0].text_input("CAP")
        cit_n = ca_row[1].text_input("Città *")
        pro_n = ca_row[2].text_input("Prov.")
        no_n = st.text_area("Note iniziali")

        if st.form_submit_button("✅ CREA E SALVA"):
            if nn and (cit_n or pos_new):
                ind_testo = f"{via_n}, {cap_n} {cit_n} {pro_n}".strip(", ")
                lat, lon = (pos_new['latitude'], pos_new['longitude']) if pos_new else (None, None)
                if not lat:
                    coords = get_coords(ind_testo)
                    if coords: lat, lon = coords
                if lat:
                    nuovo = {'nome cliente': nn, 'indirizzo': ind_testo, 'referente': '', 'telefono': ut, 'cellulare': uc, 'mail': um, 'note': no_n, 'visitare': 'SI', 'frequenza (giorni)': nf, 'latitude': lat, 'longitude': lon, 'ultima visita': pd.Timestamp('2000-01-01'), 'appuntamento': pd.NaT}
                    st.session_state.df_master = pd.concat([st.session_state.df_master, pd.DataFrame([nuovo])], ignore_index=True)
                    if save_to_gsheets(st.session_state.df_master): st.success("Salvato correttamente!"); st.rerun()
                else: st.error("Posizione non trovata. Inserisci almeno la città.")
...
# --- TAB: PARAMETRI (RIPRISTINATI) ---
    ...
    with c_gps:
        if st.button("🎯 USA GPS ATTUALE"):
            gps_p = streamlit_js_eval(js_expressions='navigator.geolocation.getCurrentPosition((pos) => { return pos.coords; })', target_id='gps_param_v13')
            if gps_p:
