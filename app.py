...
st.set_page_config(page_title="Giro Visite CRM Pro", layout="wide", initial_sidebar_state="collapsed")
...
def get_coords(address):
    try:
        geolocator = Nominatim(user_agent="giro_visite_v20_italy")
...
if st.session_state.active_tab == "🚀 Giro Oggi":
    st.header("📍 Tappe suggerite per oggi")
            ...
            with c1:
                for t in tappe:
                    with st.container(border=True):
                        ...
                        cols = st.columns(4)
                        cols[0].link_button("🚗", f"http://maps.google.com/?daddr={t['latitude']},{t['longitude']}")
...
# --- TAB: MAPPA ---
    ...
    filtro = st.radio("Mostra:", ["Tutti i Clienti", "Visitati", "Mai Visitati"], horizontal=True)
...
# --- TAB: ANAGRAFICA ---
    ...
    if scelto:
        ...
        c_act = st.columns(4)
        c_act[0].link_button("🚗 NAVIGA", f"http://maps.google.com/?daddr={d['latitude']},{d['longitude']}")
        if d.get('cellulare'): c_act[1].link_button("📱 CELL", f"tel:{d['cellulare']}")
        ...
        with st.expander("🗑️ ELIMINA CLIENTE"):
            if st.checkbox("Confermo eliminazione") and st.button("❌ ELIMINA"):
...
# --- TAB: NUOVO CLIENTE ---
    ...
    # PULSANTE GPS FUORI DAL FORM CON LOGICA FORZATA PER IOS
    st.subheader("📍 Geolocalizzazione GPS")
    pos_new = streamlit_js_eval(js_expressions='navigator.geolocation.getCurrentPosition((pos) => { return pos.coords; }, (err) => { return err; }, {enableHighAccuracy:true})', target_id='gps_pro_v20')
    
    if pos_new:
        if 'latitude' in pos_new:
            st.success(f"✅ GPS Attivo: {pos_new['latitude']}, {pos_new['longitude']}")
        else:
            st.warning("⚠️ L'iPad ha bloccato la richiesta. Controlla il lucchetto vicino all'indirizzo in alto su Chrome.")
    ...
    with st.form("new_full"):
        ...
        if st.form_submit_button("✅ CREA E SALVA"):
            if nn and (cit or pos_new):
                ...
                lat, lon = (pos_new['latitude'], pos_new['longitude']) if (pos_new and 'latitude' in pos_new) else (None, None)
                if not lat:
                    coords = get_coords(ind_comp)
                    if coords: lat, lon = coords
                if lat:
                    nuovo = {'nome cliente': nn, 'indirizzo': ind_comp, 'telefono': ut, 'cellulare': uc, 'mail': um, 'note': no, 'visitare': 'SI', 'frequenza (giorni)': nf, 'latitude': lat, 'longitude': lon, 'ultima visita': pd.Timestamp('2000-01-01')}
                    st.session_state.df_master = pd.concat([st.session_state.df_master, pd.DataFrame([nuovo])], ignore_index=True)
                    save_to_gsheets(st.session_state.df_master); st.rerun()
                else: st.error("Errore: indirizzo non trovato. Scrivi meglio la città.")
...
# --- TAB: PARAMETRI ---
    ...
    with c_gps:
        gps_p = streamlit_js_eval(js_expressions='navigator.geolocation.getCurrentPosition((pos) => { return pos.coords; })', target_id='gps_param_v20')
        if st.button("🎯 USA POSIZIONE GPS"):
            if gps_p and 'latitude' in gps_p:
                save_config_cloud("GPS iPad", gps_p['latitude'], gps_p['longitude'])
                st.session_state.start_lat, st.session_state.start_lon, st.session_state.start_city = gps_p['latitude'], gps_p['longitude'], "GPS iPad"
                st.rerun()
    with c_city:
        nc = st.text_input("Oppure scrivi Città:", st.session_state.start_city)
        if nc != st.session_state.start_city and nc != "GPS iPad":
            co = get_coords(nc)
    ...
    st.divider()
    st.subheader("📊 Esportazione e Database")
    def to_excel(df):
        out = io.BytesIO()
