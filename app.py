    ...
    if idx_g < 5:
        ...
        if tappe:
            ...
            with c2: 
                ...
                st_folium(m_oggi, width="100%", height=600, key="map_oggi_v4")
...
# --- TAB: MAPPA ---
    ...
    filtro_tipo = st.radio("Filtro:", ["Tutti", "Visitati", "Mai Visitati"], horizontal=True, key="f_map_v4")
    ...
    if not df_m.empty:
        ...
        output = st_folium(m, width="100%", height=600, key="main_map_v4")
...
# --- TAB: ANAGRAFICA ---
    ...
    scelto = st.selectbox("Cerca cliente:", opzioni, index=idx_def, key="sel_ana_v4")
    ...
    if scelto != "":
        ...
        if pd.notnull(d['ultima visita']) and d['ultima visita'] > pd.Timestamp('2000-01-01'):
            ...
            elif gm == 0: st.warning("📅 **Prossima visita suggerita:** OGGI!")
        ...
        with st.container(border=True):
            ...
            if st.button("✅ APPENA VISITATO", type="primary", use_container_width=True, key="btn_visit_v4"):
                st.session_state.show_quick_report = True
            if st.session_state.get('show_quick_report', False):
                cr1, cr2 = st.columns([1, 2])
                dv = cr1.date_input("Data visita:", value=ora_italiana.date(), key="dv_in_v4")
                rt = st.text_area("Report incontri:", placeholder="Cosa è emerso?", key="rt_in_v4")
                if st.button("💾 SALVA IN STORICO", key="save_rep_v4"):
                    ...
                    save_to_gsheets(st.session_state.df_master); st.session_state.show_quick_report = False; st.rerun()
                if st.button("Annulla", key="can_rep_v4"): st.session_state.show_quick_report = False; st.rerun()

        st.divider()
        with st.form("edit_anag_v4"):
            c1, c2 = st.columns(2)
...
# --- TAB: NUOVO CLIENTE ---
    ...
    if st.button("🎯 ACQUISISCI POSIZIONE GPS ATTUALE", key="gps_new_v4"):
        pos = streamlit_js_eval(js_expressions='navigator.geolocation.getCurrentPosition((p)=>{return p.coords;})', target_id='gps_n_v4')
        if pos: st.session_state.new_coords = (pos['latitude'], pos['longitude']); st.success("Coordinate GPS acquisite!")
    with st.form("new_full_v4"):
        c1, c2 = st.columns(2)
...
# --- TAB: PARAMETRI ---
    ...
    if st.button("🎯 USA GPS ATTUALE COME BASE", key="gps_base_v4"):
        gps = streamlit_js_eval(js_expressions='navigator.geolocation.getCurrentPosition((p)=>{return p.coords;})', target_id='gps_p_v4')
        if gps: save_config_cloud("GPS", gps['latitude'], gps['longitude']); st.rerun()
    nc = st.text_input("Città base:", st.session_state.start_city)
    if nc != st.session_state.start_city:
        co = get_coords(nc)
        if co: save_config_cloud(nc, co[0], co[1]); st.rerun()
    st.divider()
    ...
    def to_excel(df):
        ...
        return out.getvalue()
    st.download_button("📥 Database Excel", data=to_excel(st.session_state.df_master), file_name="crm.xlsx", key="dl_v4")
    if st.button("🔄 Forza Ricarica Cloud", key="rel_v4"): st.cache_data.clear(); st.rerun()
...
# --- TAB: AGENDA ---
    ...
    if col_p.button("⬅️ Precedente", key="agg_p_v4"): st.session_state.current_week_index -= 1; st.rerun()
    ...
    if col_n.button("Successiva ➡️", key="agg_n_v4"): st.session_state.current_week_index += 1; st.rerun()
    st.divider()
    cols = st.columns(5)
        ...
        with cols[i]:
            ...
            for t in agenda[st.session_state.current_week_index][i]:
                with st.container(border=True):
                    ...
                    if st.button(t['nome cliente'], key=f"ag_v4_{st.session_state.current_week_index}_{i}_{t['nome cliente']}", use_container_width=True):
                        st.session_state.cliente_selezionato = t['nome cliente']; st.session_state.active_tab = "👤 Anagrafica"; st.rerun()
